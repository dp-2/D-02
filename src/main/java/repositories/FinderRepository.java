...
public interface FinderRepository extends JpaRepository<Finder, Integer> {
	...
	@Query("select min(f.parades.size)*1.0 from Finder f")
	...
	@Query("select sqrt(((select sum(f1.parades.size) from Finder f1) - (select avg(f2.parades.size) from Finder f2)) * ((select sum(f1.parades.size) from Finder f1) - (select avg(f2.parades.size) from Finder f2)) / count(f)) from Finder f")
